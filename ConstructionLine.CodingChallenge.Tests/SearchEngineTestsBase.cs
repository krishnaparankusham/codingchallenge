    ...
    public class SearchEngineTestsBase
    {
        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts.Count(s => s.Size.Id == size.Id && ApplySearchOptions(searchOptions, s));
        ...
        protected static void AssertColorCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
        {
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts.Count(c => c.Color.Id == color.Id && ApplySearchOptions(searchOptions, c));
        ...
        private static bool ApplySearchOptions(SearchOptions searchOptions, Shirt shirt)
        {
            return (!searchOptions.Sizes.Any() || searchOptions.Sizes.Any(c => c.Id == shirt.Size.Id)) &&
                   (!searchOptions.Colors.Any() || searchOptions.Colors.Any(c => c.Id == shirt.Color.Id));
        }
    }
}